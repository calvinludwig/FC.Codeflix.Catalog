...
public class GetCategoryTest(GetCategoryTestFixture fixture)
{
    ...
    [Fact(DisplayName = nameof(ShouldBeAbleToGetCategory))]
    [Trait("Integration/Application", "GetCategory - Use Cases")]
    public async Task ShouldBeAbleToGetCategory()
    {
    ...
    [Fact(DisplayName = nameof(ShouldThrowNotFoundExceptionWhenCategoryDoesntExist))]
    [Trait("Integration/Application", "GetCategory - Use Cases")]
    public async Task ShouldThrowNotFoundExceptionWhenCategoryDoesntExist()
    {
        var exampleCategory = _fixture.GetExampleCategory();
        var dbContext = _fixture.CreateDbContext();
        dbContext.Categories.Add(exampleCategory);
