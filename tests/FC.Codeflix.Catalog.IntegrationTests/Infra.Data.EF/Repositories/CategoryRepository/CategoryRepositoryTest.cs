...
public class CategoryRepositoryTest
{
    ...
    [Fact(DisplayName = nameof(ShouldBeAbleToInsert))]
    ...
    public async Task ShouldBeAbleToInsert()
    {
        ...
        var dbCategory = await (_fixture.CreateDbContext(true)).Categories.FindAsync(
            exampleCategory.Id
        );
    ...
    [Fact(DisplayName = nameof(ShouldBeAbleToGet))]
    [Trait("Integration/Infra.Data", "CategoryRepository - Repositories")]
    public async Task ShouldBeAbleToGet()
    {
        var dbContext = _fixture.CreateDbContext();
        var exampleCategory = _fixture.GetExampleCategory();
    ...
    [Fact(DisplayName = nameof(ShouldThrowWhenNotFoundOnGet))]
    [Trait("Integration/Infra.Data", "CategoryRepository - Repositories")]
    public async Task ShouldThrowWhenNotFoundOnGet()
    {
    ...
    [Fact(DisplayName = nameof(ShouldBeAbleToUpdate))]
    [Trait("Integration/Infra.Data", "CategoryRepository - Repositories")]
    ...
    [Fact(DisplayName = nameof(ShouldBeAbleToDelete))]
    [Trait("Integration/Infra.Data", "CategoryRepository - Repositories")]
    ...
    [Fact(DisplayName = nameof(ShouldBeAbleToSearchAndList))]
    [Trait("Integration/Infra.Data", "CategoryRepository - Repositories")]
    public async Task ShouldBeAbleToSearchAndList()
    {
        var dbContext = _fixture.CreateDbContext();
        var exampleCategoriesList = _fixture.GetExampleCategoriesList();
        await dbContext.AddRangeAsync(exampleCategoriesList);
    ...
    [Fact(DisplayName = nameof(ShouldBeAbleToSearchAndListWhenEmpty))]
    [Trait("Integration/Infra.Data", "CategoryRepository - Repositories")]
    public async Task ShouldBeAbleToSearchAndListWhenEmpty()
    {
        var dbContext = _fixture.CreateDbContext();
        var categoryRepository = new Repository.CategoryRepository(dbContext);
        var searchInput = new SearchInput(1, 20, "", "", SearchOrder.Asc);
    ...
    [Theory(DisplayName = nameof(ShouldBeAbleToSearchAndListPaginated))]
    [Trait("Integration/Infra.Data", "CategoryRepository - Repositories")]
    [InlineData(10, 1, 5, 5)]
    [InlineData(10, 2, 5, 5)]
    [InlineData(7, 2, 5, 2)]
    [InlineData(7, 3, 5, 0)]
    public async Task ShouldBeAbleToSearchAndListPaginated(
        int qtdItems,
        int page,
        int perPage,
        int expectedQtdItems
    )
    {
        var dbContext = _fixture.CreateDbContext();
        var exampleCategoriesList = _fixture.GetExampleCategoriesList(qtdItems);
        await dbContext.AddRangeAsync(exampleCategoriesList);
        await dbContext.SaveChangesAsync(CancellationToken.None);
        var categoryRepository = new Repository.CategoryRepository(dbContext);
        ...
        output.CurrentPage.Should().Be(searchInput.Page);
        output.PerPage.Should().Be(searchInput.PerPage);
        output.Total.Should().Be(qtdItems);
        output.Items.Should().HaveCount(expectedQtdItems);
        foreach (var outputItem in output.Items)
        {
            var exampleItem = exampleCategoriesList.Find(category => category.Id == outputItem.Id);
            exampleItem.Should().NotBeNull();
    ...
    [Theory(DisplayName = nameof(ShouldBeAbleToSearchAndListPaginatedByText))]
    [Trait("Integration/Infra.Data", "CategoryRepository - Repositories")]
    ...
    [InlineData("Robots", 1, 3, 2, 2)]
    public async Task ShouldBeAbleToSearchAndListPaginatedByText(
        string search,
        int page,
        int perPage,
        int expectedQtdItems,
        int expectedQtdTotalItems
    )
    {
        var dbContext = _fixture.CreateDbContext();
        ...
        output.CurrentPage.Should().Be(searchInput.Page);
        output.PerPage.Should().Be(searchInput.PerPage);
        output.Total.Should().Be(expectedQtdTotalItems);
        output.Items.Should().HaveCount(expectedQtdItems);
        foreach (var outputItem in output.Items)
        {
            var exampleItem = exampleCategoriesList.Find(category => category.Id == outputItem.Id);
    ...
    [Theory(DisplayName = nameof(ShouldBeAbleToSearchAndListOrdered))]
    [Trait("Integration/Infra.Data", "CategoryRepository - Repositories")]
    [InlineData("name", "asc")]
